...
bool DriverCore::validate_target(
  ...
  for (std::size_t i = 0; i < target.size(); ++i) {
    if (!std::isfinite(target[i]) || target[i] < minimum[i] || target[i] > maximum[i]) {
      return false;
    }
  }
  return true;
...
bool DriverCore::connect_and_enable(
    ...
    int left_ratio,
    int right_ratio) {
  if (left_ratio < 1 || left_ratio > 100 || right_ratio < 1 || right_ratio > 100) {
    return false;
  }
  ...
  const int link_result = FX_L1_System_Link(ip[0], ip[1], ip[2], ip[3], FX_LOG_ALL_FLAG);
  if (link_result < 0) {
    return false;
  }
  linked_ = true;
  ...
  if (FX_L1_State_SwitchToPositionMode(
          FX_OBJ_ARM0, kModeTimeoutMs, left_ratio, left_ratio) != 0) {
    return fail_and_disconnect();
  }
  if (FX_L1_State_SwitchToPositionMode(
          FX_OBJ_ARM1, kModeTimeoutMs, right_ratio, right_ratio) != 0) {
    return fail_and_disconnect();
  }
  if (FX_L1_Runtime_SetSpeedRatio(kThreadId, FX_OBJ_ARM0, left_ratio, left_ratio) != 0) {
    return fail_and_disconnect();
  }
  if (FX_L1_Runtime_SetSpeedRatio(kThreadId, FX_OBJ_ARM1, right_ratio, right_ratio) != 0) {
    return fail_and_disconnect();
  }
...
bool DriverCore::command_allowed() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return linked_ && position_ready_;
}
...
bool DriverCore::hold_current() {
  ...
  if (!position_ready_) {
    if (FX_L1_State_SwitchToPositionMode(
            FX_OBJ_ARM0, kModeTimeoutMs, left_ratio_, left_ratio_) != 0) {
      return false;
    }
    if (FX_L1_State_SwitchToPositionMode(
            FX_OBJ_ARM1, kModeTimeoutMs, right_ratio_, right_ratio_) != 0) {
      return false;
    }
    if (FX_L1_Runtime_SetSpeedRatio(
            kThreadId, FX_OBJ_ARM0, left_ratio_, left_ratio_) != 0) {
      return false;
    }
    if (FX_L1_Runtime_SetSpeedRatio(
            kThreadId, FX_OBJ_ARM1, right_ratio_, right_ratio_) != 0) {
      return false;
    }
  }

  const bool left_ok =
      FX_L1_Runtime_SetJointPosCmd(kThreadId, FX_OBJ_ARM0, left_deg.data()) == 0;
  const bool right_ok =
...
bool DriverCore::send_position(Arm arm, const JointArray& target) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!linked_ || !position_ready_) {
    return false;
  }
  auto command = ros_radians_to_sdk_degrees(target);
...
std::optional<DriverCore::DualArmState> DriverCore::read_state() const {
  ...
  for (std::size_t i = 0; i < left_position_degrees.size(); ++i) {
    left_position_degrees[i] = feedback->m_ARMS[0].m_ARM_OUT.m_ARM_FBK_Joint_Pos[i];
    right_position_degrees[i] = feedback->m_ARMS[1].m_ARM_OUT.m_ARM_FBK_Joint_Pos[i];
    left_velocity_degrees[i] = feedback->m_ARMS[0].m_ARM_OUT.m_ARM_FBK_Joint_Vel[i];
    right_velocity_degrees[i] = feedback->m_ARMS[1].m_ARM_OUT.m_ARM_FBK_Joint_Vel[i];
  }
